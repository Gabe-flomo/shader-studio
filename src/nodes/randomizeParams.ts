 ...
 *   window that's that fraction of the range, centred on the current value and shifted to stay
 *   inside it. 1 = anywhere in the range; 0.1 = a small nudge.
 */
export function randomizedParams(node: GraphNode, def: NodeDefinition, rand: () => number = Math.random): Record<string, unknown> {
  ...
  const amount = randomizeAmount(node);
  const between = (lo: number, hi: number, pd: ParamDef, current: unknown) => pick(lo, hi, current, amount, rand, pd);
  ...
  for (const [key, pd] of Object.entries(def.paramDefs ?? {})) {
    ...
    if (pd.type === 'float' || pd.type === 'int') {
      ...
      out[key] = ['r', 'g', 'b'].map((c, i) => (wired(`${key}_${c}`) ? cur[i] ?? 0 : between(lo, hi, pd, cur[i])));
    }
  }
  return out;
}
...
/** Randomize strength for a node, 0.01–1 (1 when unset) */
export function randomizeAmount(node: GraphNode): number {
  const a = node.params.__randAmount;
  return typeof a === 'number' && a > 0 ? Math.min(1, Math.max(0.01, a)) : 1;
}

/** A value in [lo, hi] drawn from a window `amount` of the range wide, centred on `current` */
function pick(lo: number, hi: number, current: unknown, amount: number, rand: () => number, pd: ParamDef): number {
  const width = (hi - lo) * amount;
  const centre = typeof current === 'number' && Number.isFinite(current) ? current : (lo + hi) / 2;
  const start = Math.min(hi - width, Math.max(lo, centre - width / 2));
  return snap(start + rand() * width, pd);
}
