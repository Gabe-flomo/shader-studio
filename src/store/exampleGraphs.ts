...
export const EXAMPLE_GRAPHS: Record<string, { label: string; nodes: GraphNode[]; counter: number }> = {
  ...
  // Grid: Gravity — cells drawn toward mouse; alternating columns filtered by cellFilter
  gridGravity: {
    label: 'Grid: Gravity',
    counter: 6,
    nodes: [
      { id: 'gg_uv', type: 'uv', position: { x: 60, y: 240 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gg_time', type: 'time', position: { x: 60, y: 400 },
        inputs: {}, outputs: { time: { type: 'float', label: 'Time' } }, params: {} },
      { id: 'gg_grid', type: 'gridLayout', position: { x: 280, y: 240 },
        ...
        params: { columns: 16 } },
      // Traveler: sweeps left-to-right continuously on a non-active (even) row
      { id: 'gg_trav', type: 'exprNode', position: { x: 400, y: 420 },
        inputs: {
          t:   { type: 'float', label: 't',   connection: { nodeId: 'gg_time', outputKey: 'time'         } },
          asp: { type: 'float', label: 'asp', connection: { nodeId: 'gg_grid', outputKey: 'aspect_ratio' } },
        },
        outputs: { result: { type: 'vec2', label: 'Result (vec2)' } },
        params: { inputs: [{ name: 't', type: 'float', slider: null }, { name: 'asp', type: 'float', slider: null }],
          outputType: 'vec2', lines: [], result: 'vec2((fract(t * 0.25) * 2.0 - 1.0) * asp, 0.25)',
          expr: 'a', label: 'traveler' } },
      // Draw: odd rows show ghost circle + displaced dot + connection line; traveler glow is world-space
      { id: 'gg_draw', type: 'exprNode', position: { x: 760, y: 300 },
        inputs: {
          cuv:  { type: 'vec2',  label: 'cuv',  connection: { nodeId: 'gg_grid', outputKey: 'cellUV'     } },
          cid:  { type: 'vec2',  label: 'cid',  connection: { nodeId: 'gg_grid', outputKey: 'cellID'     } },
          cc:   { type: 'vec2',  label: 'cc',   connection: { nodeId: 'gg_grid', outputKey: 'cellCenter' } },
          uv:   { type: 'vec2',  label: 'uv',   connection: { nodeId: 'gg_uv',   outputKey: 'uv'         } },
          trav: { type: 'vec2',  label: 'trav', connection: { nodeId: 'gg_trav', outputKey: 'result'     } },
        },
        outputs: { result: { type: 'vec3', label: 'Result (vec3)' } },
        params: { inputs: [
            { name: 'cuv',  type: 'vec2', slider: null },
            { name: 'cid',  type: 'vec2', slider: null },
            { name: 'cc',   type: 'vec2', slider: null },
            { name: 'uv',   type: 'vec2', slider: null },
            { name: 'trav', type: 'vec2', slider: null },
          ],
          outputType: 'vec3',
          lines: [
            { lhs: 'vec2 dW',     op: '=',  rhs: 'trav - cc' },
            { lhs: 'float dstW',  op: '=',  rhs: 'length(dW)' },
            { lhs: 'float attr',  op: '=',  rhs: 'smoothstep(0.7, 0.0, dstW)' },
            { lhs: 'vec2 dirW',   op: '=',  rhs: 'dW / max(dstW, 0.001)' },
            { lhs: 'vec2 dotC',   op: '=',  rhs: 'dirW * attr * 0.4' },
            { lhs: 'float h',     op: '=',  rhs: 'clamp(dot(cuv, dotC) / (dot(dotC, dotC) + 0.0001), 0.0, 1.0)' },
            { lhs: 'float lnDst', op: '=',  rhs: 'length(cuv - dotC * h)' },
            { lhs: 'float rowMask',op: '=',  rhs: 'step(0.5, mod(cid.y, 2.0))' },
            { lhs: 'float ghost', op: '=',  rhs: 'smoothstep(0.16, 0.14, length(cuv)) * 0.25' },
            { lhs: 'float dotV',  op: '=',  rhs: 'smoothstep(0.16, 0.14, length(cuv - dotC))' },
            { lhs: 'float lineV', op: '=',  rhs: 'smoothstep(0.012, 0.004, lnDst) * attr * 0.6' },
            { lhs: 'float gDist', op: '=',  rhs: 'length(uv - trav)' },
            { lhs: 'float tGlow', op: '=',  rhs: 'exp(-gDist * 14.0) * 0.7 + smoothstep(0.07, 0.05, gDist)' },
          ],
          result: '(ghost * vec3(0.3,0.3,0.4) + (dotV + lineV) * vec3(0.85,0.9,1.0)) * rowMask + tGlow * vec3(0.1,0.9,0.7)',
          expr: 'a', label: 'draw' } },
      { id: 'gg_out', type: 'output', position: { x: 1020, y: 300 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gg_draw', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Metaballs — 5 animated Gaussian blobs, summed exponential field, metaball threshold
  gridMetaballs: {
    label: 'Grid: Metaballs',
    counter: 6,
    nodes: [
      { id: 'gm_uv', type: 'uv', position: { x: 60, y: 280 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      // Field: sum Gaussian contributions from 5 animated blobs (t auto-injected as u_time)
      { id: 'gm_field', type: 'exprNode', position: { x: 320, y: 280 },
        inputs: { uv: { type: 'vec2', label: 'uv', connection: { nodeId: 'gm_uv', outputKey: 'uv' } } },
        outputs: { result: { type: 'float', label: 'Result (float)' } },
        params: { inputs: [{ name: 'uv', type: 'vec2', slider: null }],
          outputType: 'float',
          lines: [
            { lhs: 'float k',  op: '=',  rhs: '14.0' },
            { lhs: 'float f',  op: '=',  rhs: '0.0' },
            { lhs: 'vec2 p0',  op: '=',  rhs: 'vec2(sin(t*1.1)*0.55, cos(t*0.71)*0.35)' },
            { lhs: 'vec2 p1',  op: '=',  rhs: 'vec2(cos(t*0.83)*0.48, sin(t*1.23+1.0)*0.38)' },
            { lhs: 'vec2 p2',  op: '=',  rhs: 'vec2(sin(t*0.61+2.1)*0.52, cos(t*0.93)*0.30)' },
            { lhs: 'vec2 p3',  op: '=',  rhs: 'vec2(cos(t*1.31)*0.40, sin(t*0.55+3.0)*0.42)' },
            { lhs: 'vec2 p4',  op: '=',  rhs: 'vec2(sin(t*0.97+1.5)*0.60, cos(t*1.13)*0.28)' },
            { lhs: 'f', op: '+=', rhs: 'exp(-dot(uv-p0,uv-p0)*k)' },
            { lhs: 'f', op: '+=', rhs: 'exp(-dot(uv-p1,uv-p1)*k)' },
            { lhs: 'f', op: '+=', rhs: 'exp(-dot(uv-p2,uv-p2)*k)' },
            { lhs: 'f', op: '+=', rhs: 'exp(-dot(uv-p3,uv-p3)*k)' },
            { lhs: 'f', op: '+=', rhs: 'exp(-dot(uv-p4,uv-p4)*k)' },
          ],
          result: 'f',
          expr: 'a', label: 'field' } },
      { id: 'gm_thr', type: 'metaballThreshold', position: { x: 600, y: 280 },
        inputs: { field: { type: 'float', label: 'Field', connection: { nodeId: 'gm_field', outputKey: 'result' } } },
        outputs: { blob: { type: 'float', label: 'Blob' }, edge: { type: 'float', label: 'Edge' } },
        params: { threshold: 0.8, softness: 0.07 } },
      { id: 'gm_pal', type: 'palettePreset', position: { x: 600, y: 460 },
        inputs: { t: { type: 'float', label: 'T', connection: { nodeId: 'gm_field', outputKey: 'result' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } },
        params: {} },
      { id: 'gm_draw', type: 'exprNode', position: { x: 860, y: 360 },
        inputs: {
          blob: { type: 'float', label: 'blob', connection: { nodeId: 'gm_thr',   outputKey: 'blob'  } },
          edge: { type: 'float', label: 'edge', connection: { nodeId: 'gm_thr',   outputKey: 'edge'  } },
          col:  { type: 'vec3',  label: 'col',  connection: { nodeId: 'gm_pal',   outputKey: 'color' } },
        },
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'blob', type: 'float', slider: null }, { name: 'edge', type: 'float', slider: null }, { name: 'col', type: 'vec3', slider: null }],
          outputType: 'vec3', lines: [], result: 'blob * col + edge * vec3(1.5,1.2,0.5)', expr: 'a', label: 'colorize' } },
      { id: 'gm_out', type: 'output', position: { x: 1100, y: 360 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gm_draw', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },
