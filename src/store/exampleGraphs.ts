...
export const EXAMPLE_GRAPHS: Record<string, { label: string; nodes: GraphNode[]; counter: number }> = {
  ...
  // ── Chladni Field — the one-node fast path ────────────────────────────────
  // n, m, Mix and a Square/Circular geometry switch, all in one node, piped
  // straight into Field to Lines. The quickest way to a good-looking pattern.
  chladniFieldQuickDemo: {
    label: 'Chladni Field (Quick)',
    counter: 4,
    nodes: [
      { id: 'uv_0', type: 'uv', position: { x: 40, y: 220 }, inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      {
        id: 'field_1', type: 'chladniField', position: { x: 280, y: 220 },
        ...
        params: { n: 6.0, m: 4.0, mix: 1.0, scale: 1.0, geometry: 'square' },
      },
      {
        id: 'lines_2', type: 'fieldToLines', position: { x: 540, y: 220 },
      ...
      {
        id: 'output_3', type: 'output', position: { x: 780, y: 220 },
  ...
  // ── Chladni Superposition — the composable "in-between" path ─────────────
  // Scatter (organic UV jitter) → Chladni Superposition (3 stacked modes,
  // grown from the default single mode via the Terms param — no manual Wave
  // Term wiring) → Field to Lines. Every node here is reusable outside
  // Chladni too.
  chladniComposableDemo: {
    ...
    nodes: [
      ...
      { id: 'time_1', type: 'time', position: { x: 40,  y: 440 }, inputs: {}, outputs: { time: { type: 'float', label: 'Time' } }, params: {} },
      {
        id: 'scatter_2', type: 'scatter', position: { x: 260, y: 340 },
        inputs: {
          uv:   { type: 'vec2',  label: 'UV',   connection: { nodeId: 'uv_0',   outputKey: 'uv'   } },
          time: { type: 'float', label: 'Time', connection: { nodeId: 'time_1', outputKey: 'time' } },
        },
      ...
      {
        id: 'super_3', type: 'chladniSuperposition', position: { x: 520, y: 260 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'scatter_2', outputKey: 'field' } } },
        outputs: { field: { type: 'float', label: 'Field' }, uv: { type: 'vec2', label: 'UV (scaled)' } },
        params: {
          scale: 1.0, geometry: 'square', terms: '3',
          n1: 5.0, m1: 6.0,
          n2: 3.0, m2: 8.0, w2: 0.4,
          n3: 7.0, m3: 2.0, w3: 0.2,
        },
      },
      {
        id: 'lines_4', type: 'fieldToLines', position: { x: 780, y: 260 },
        inputs: { field: { type: 'float', label: 'Field', connection: { nodeId: 'super_3', outputKey: 'field' } } },
        outputs: { density: { type: 'float', label: 'Density' }, color: { type: 'vec3', label: 'Color' } },
        params: { line_width: 1.5, aa: 1.0, brightness: 1.2 },
      },
      {
        id: 'output_5', type: 'output', position: { x: 1020, y: 260 },
  ...
  // ── Wave Term — fully manual, fully wirable 2-mode mix ────────────────────
  // The raw compositional path: two Wave Term nodes (n/m swapped, matching the
  // classic Chladni cos(n)cos(m) − cos(m)cos(n) formula) combined with Subtract.
  // Every n/m here is a real wirable socket — use this when you need to
  // animate/automate individual mode numbers independently, which the compact
  // Chladni Field / Superposition nodes deliberately trade away for brevity.
  waveTermManualMixDemo: {
    label: 'Wave Term (Manual Mix)',
    counter: 5,
    nodes: [
      { id: 'uv_0', type: 'uv', position: { x: 40, y: 220 }, inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      {
        id: 'wave_1', type: 'waveTerm', position: { x: 280, y: 100 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'uv_0', outputKey: 'uv' } } },
        outputs: { value: { type: 'float', label: 'Value' }, uv: { type: 'vec2', label: 'UV (scaled)' } },
        params: { n: 6.0, m: 4.0, scale: 1.0 },
      },
      {
        id: 'wave_2', type: 'waveTerm', position: { x: 280, y: 320 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'uv_0', outputKey: 'uv' } } },
        outputs: { value: { type: 'float', label: 'Value' }, uv: { type: 'vec2', label: 'UV (scaled)' } },
        params: { n: 4.0, m: 6.0, scale: 1.0 },
      },
      {
        id: 'sub_3', type: 'subtract', position: { x: 540, y: 210 },
        inputs: {
          a: { type: 'float', label: 'A', connection: { nodeId: 'wave_1', outputKey: 'value' } },
          b: { type: 'float', label: 'B', connection: { nodeId: 'wave_2', outputKey: 'value' } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { b: 0.0 },
      },
      {
        id: 'lines_4', type: 'fieldToLines', position: { x: 780, y: 210 },
        inputs: { field: { type: 'float', label: 'Field', connection: { nodeId: 'sub_3', outputKey: 'result' } } },
        outputs: { density: { type: 'float', label: 'Density' }, color: { type: 'vec3', label: 'Color' } },
        params: { line_width: 1.5, aa: 1.0, brightness: 1.2 },
      },
      {
        id: 'output_5', type: 'output', position: { x: 1020, y: 210 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'lines_4', outputKey: 'color' } } },
        outputs: {}, params: {},
      },
    ],
  ...
  // ── Quantize → Chladni Field — stepped modes without baking in integer steps ──
  // A slow Sine LFO sweeps continuously; Quantize snaps it to whole-number
  // steps before it drives Chladni Field's (now-wirable) n socket — discrete
  // mode jumps, built by composing a generic node rather than hardcoding an
  // integer-only range into Chladni Field itself.
  quantizeChladniDemo: {
    label: 'Quantize → Chladni Field',
    counter: 5,
    nodes: [
      { id: 'uv_0',   type: 'uv',   position: { x: 40,  y: 260 }, inputs: {}, outputs: { uv:   { type: 'vec2',  label: 'UV'   } }, params: {} },
      { id: 'time_1', type: 'time', position: { x: 40,  y: 440 }, inputs: {}, outputs: { time: { type: 'float', label: 'Time' } }, params: {} },
      {
        ...
        params: { freq: 0.15, phase: 0.0, amplitude: 3.5, offset: 4.5 },
      },
      {
        id: 'quant_3', type: 'quantize', position: { x: 500, y: 380 },
        inputs: { input: { type: 'float', label: 'Input', connection: { nodeId: 'lfo_2', outputKey: 'value' } } },
      ...
      },
      {
        ...
        inputs: {
          uv: { type: 'vec2',  label: 'UV', connection: { nodeId: 'uv_0',    outputKey: 'uv'     } },
          n:  { type: 'float', label: 'n',  connection: { nodeId: 'quant_3', outputKey: 'output' } },
        },
        outputs: { field: { type: 'float', label: 'Field' }, uv: { type: 'vec2', label: 'UV (scaled)' } },
        params: { n: 6.0, m: 4.0, mix: 1.0, scale: 1.0, geometry: 'square' },
      },
  ...
  // ── Chladni Field — Circular (fixed formula) ──────────────────────────────
  // f(r,θ) = cos(n·θ + mix·π)·cos(m·π·r), masked to the unit disc — ported
  // directly from the validated reference instead of the earlier square-style
  // two-term approximation. Concentric rings (m) with angular petals (n).
  chladniFieldCircularDemo: {
    label: 'Chladni Field (Circular)',
    counter: 4,
    nodes: [
      { id: 'uv_0', type: 'uv', position: { x: 40, y: 220 }, inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      {
        ...
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'uv_0', outputKey: 'uv' } } },
        outputs: { field: { type: 'float', label: 'Field' }, uv: { type: 'vec2', label: 'UV (scaled)' } },
        params: { n: 5.0, m: 4.0, mix: 1.0, scale: 1.0, geometry: 'circular' },
      },
      {
        id: 'lines_2', type: 'fieldToLines', position: { x: 540, y: 220 },
        inputs: { field: { type: 'float', label: 'Field', connection: { nodeId: 'field_1', outputKey: 'field' } } },
  ...
  // Grid: Rings — fract(dist_to_center) gives repeating concentric ring bands per cell
  gridBasic: {
    label: 'Grid: Rings',
    counter: 5,
    nodes: [
      { id: 'gb_uv0', type: 'uv', position: { x: 60, y: 200 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gb_grid1', type: 'gridLayout', position: { x: 280, y: 200 },
        ...
        params: { columns: 10 } },
      { id: 'gb_rings2', type: 'exprNode', position: { x: 520, y: 200 },
        inputs: { dtc: { type: 'float', label: 'dtc', connection: { nodeId: 'gb_grid1', outputKey: 'dist_to_center' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'dtc', type: 'float', slider: null }],
          outputType: 'float', lines: [], result: 'fract(dtc * 0.4)',
          expr: 'a', label: 'rings' } },
      { id: 'gb_f2v3', type: 'floatToVec3', position: { x: 740, y: 200 },
        inputs: { input: { type: 'float', label: 'Float', connection: { nodeId: 'gb_rings2', outputKey: 'result' } } },
        outputs: { rgb: { type: 'vec3', label: 'Color' } }, params: {} },
      { id: 'gb_out4', type: 'output', position: { x: 960, y: 200 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gb_f2v3', outputKey: 'rgb' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Color Circles — soft-edge circles colored by distance from origin
  gridCircles: {
    label: 'Grid: Color Circles',
    counter: 6,
    nodes: [
      { id: 'gc_uv0', type: 'uv', position: { x: 60, y: 200 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gc_grid1', type: 'gridLayout', position: { x: 280, y: 200 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'gc_uv0', outputKey: 'uv' } } },
        outputs: {
          cellUV:         { type: 'vec2',  label: 'Cell UV' },
          dist_to_center: { type: 'float', label: 'Dist to Center' },
          cellID:         { type: 'vec2',  label: 'Cell ID' },
        ...
        params: { columns: 10 } },
      { id: 'gc_mask2', type: 'exprNode', position: { x: 520, y: 200 },
        inputs: { cuv: { type: 'vec2', label: 'cuv', connection: { nodeId: 'gc_grid1', outputKey: 'cellUV' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'cuv', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: '1.0 - smoothstep(0.3, 0.42, length(cuv))',
          expr: 'a', label: 'circle mask' } },
      { id: 'gc_pal3', type: 'palettePreset', position: { x: 520, y: 340 },
        inputs: { t: { type: 'float', label: 'T', connection: { nodeId: 'gc_grid1', outputKey: 'dist_to_center' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } }, params: {} },
      { id: 'gc_mul4', type: 'exprNode', position: { x: 760, y: 200 },
        inputs: {
          mask: { type: 'float', label: 'mask', connection: { nodeId: 'gc_mask2', outputKey: 'result' } },
          col:  { type: 'vec3',  label: 'col',  connection: { nodeId: 'gc_pal3',  outputKey: 'color'  } },
        },
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'mask', type: 'float', slider: null }, { name: 'col', type: 'vec3', slider: null }],
          outputType: 'vec3', lines: [], result: 'mask * col', expr: 'a', label: 'colorize' } },
      { id: 'gc_out5', type: 'output', position: { x: 980, y: 200 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gc_mul4', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Wave — WaveRadius drives circle size per cell, rippling out from origin
  gridWave: {
    label: 'Grid: Wave',
    counter: 7,
    nodes: [
      { id: 'gw_uv0', type: 'uv', position: { x: 60, y: 200 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gw_grid1', type: 'gridLayout', position: { x: 280, y: 200 },
        ...
        params: { columns: 10 } },
      { id: 'gw_wave2', type: 'waveRadius', position: { x: 520, y: 300 },
        inputs: { distance: { type: 'float', label: 'Distance', connection: { nodeId: 'gw_grid1', outputKey: 'dist_to_center' } } },
      ...
      { id: 'gw_draw4', type: 'exprNode', position: { x: 760, y: 200 },
        ...
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'd', type: 'float', slider: null }, { name: 'wr', type: 'float', slider: null }],
          outputType: 'float', lines: [], result: '1.0 - smoothstep(wr - 0.015, wr + 0.015, d)',
          expr: 'a', label: 'draw' } },
      { id: 'gw_f2v5', type: 'floatToVec3', position: { x: 980, y: 200 },
        inputs: { input: { type: 'float', label: 'Float', connection: { nodeId: 'gw_draw4', outputKey: 'result' } } },
        outputs: { rgb: { type: 'vec3', label: 'Color' } }, params: {} },
      { id: 'gw_out6', type: 'output', position: { x: 1200, y: 200 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gw_f2v5', outputKey: 'rgb' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Pulse — uniform pulsing that crosses cell boundaries; demonstrates NeighborDist
  gridNeighborDots: {
    label: 'Grid: Pulse',
    counter: 7,
    nodes: [
      { id: 'nd1_uv0', type: 'uv', position: { x: 60, y: 200 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'nd1_grid1', type: 'gridLayout', position: { x: 280, y: 200 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'nd1_uv0', outputKey: 'uv' } } },
        outputs: {
          cellUV:         { type: 'vec2',  label: 'Cell UV' },
          dist_to_center: { type: 'float', label: 'Dist to Center' },
          cellID:         { type: 'vec2',  label: 'Cell ID' },
        ...
        params: { columns: 7 } },
      // distance unconnected → defaults to 0.0, giving uniform time-based radius
      { id: 'nd1_wave2', type: 'waveRadius', position: { x: 520, y: 340 },
        inputs: {},
        outputs: { wave_radius: { type: 'float', label: 'Wave Radius' } },
      ...
      { id: 'nd1_draw4', type: 'exprNode', position: { x: 760, y: 200 },
        inputs: {
          ...
          wr: { type: 'float', label: 'wr', connection: { nodeId: 'nd1_wave2', outputKey: 'wave_radius' } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'd', type: 'float', slider: null }, { name: 'wr', type: 'float', slider: null }],
          outputType: 'float', lines: [], result: '1.0 - smoothstep(wr - 0.012, wr + 0.012, d)',
          expr: 'a', label: 'draw' } },
      { id: 'nd1_f2v5', type: 'floatToVec3', position: { x: 980, y: 200 },
        inputs: { input: { type: 'float', label: 'Float', connection: { nodeId: 'nd1_draw4', outputKey: 'result' } } },
        outputs: { rgb: { type: 'vec3', label: 'Color' } }, params: {} },
      { id: 'nd1_out6', type: 'output', position: { x: 1200, y: 200 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'nd1_f2v5', outputKey: 'rgb' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Gravity — 2 tracks of circles (rows 2 & 4) displaced toward mouse; mouse is the "traveler"
  // Row mask exprNode filters out all other rows. Circles draw at displacedUV so they visually move.
  gridGravity: {
    label: 'Grid: Gravity',
    counter: 10,
    nodes: [
      { id: 'gg_uv', type: 'uv', position: { x: 60, y: 260 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gg_mouse', type: 'mouse', position: { x: 60, y: 460 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' }, x: { type: 'float', label: 'X' }, y: { type: 'float', label: 'Y' } }, params: {} },
      { id: 'gg_grid', type: 'gridLayout', position: { x: 280, y: 260 },
        ...
        params: { columns: 10 } },
      // Extract world UV.y for the soft row mask (continuous 0–1, not discrete cellID)
      { id: 'gg_uvy', type: 'extractY', position: { x: 520, y: 140 },
        inputs: { v: { type: 'vec2', label: 'Vec2', connection: { nodeId: 'gg_uv', outputKey: 'uv' } } },
        outputs: { y: { type: 'float', label: 'Y' } }, params: {} },
      // NeighborAttractCircles: checks 3×3 neighborhood so displaced circles
      // don't clip at cell boundaries — they render smoothly into adjacent cells.
      { id: 'gg_att', type: 'neighborAttractCircles', position: { x: 520, y: 320 },
        inputs: {
      ...
      { id: 'gg_fill', type: 'smoothstep', position: { x: 780, y: 320 },
        inputs: { value: { type: 'float', label: 'Value', connection: { nodeId: 'gg_att', outputKey: 'sdf' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { edge0: 0.01, edge1: -0.01 } },
      // Row filter × fill: only rows 2 and 4 are visible
      { id: 'gg_mask', type: 'exprNode', position: { x: 780, y: 160 },
        inputs: {
        ...
        params: {
          ...
          outputType: 'float', lines: [], expr: 'a', label: 'row mask',
          result: 'fill * (smoothstep(0.17,0.23,y)*smoothstep(0.33,0.27,y) + smoothstep(0.37,0.43,y)*smoothstep(0.53,0.47,y))',
        } },
      // Color driven by how much the circle was attracted (proximity to mouse)
      { id: 'gg_pal', type: 'palettePreset', position: { x: 520, y: 500 },
        inputs: { value: { type: 'float', label: 'Value', connection: { nodeId: 'gg_att', outputKey: 'attractAmount' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } },
        params: { preset: '2' } },
      { id: 'gg_col', type: 'multiplyVec3', position: { x: 1000, y: 300 },
        inputs: {
        ...
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: {} },
      { id: 'gg_out', type: 'output', position: { x: 1220, y: 300 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gg_col', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Metaballs — per-cell noise displaces each dot; Gaussian field accumulation naturally
  // produces smooth merging when noise-displaced dots come close to each other.
  gridMetaballs: {
    label: 'Grid: Metaballs',
    counter: 8,
    nodes: [
      { id: 'gm_uv', type: 'uv', position: { x: 60, y: 260 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gm_grid', type: 'gridLayout', position: { x: 280, y: 260 },
        ...
        params: { columns: 8 } },
      // NoisyGridSDF: smooth-min of noise-displaced circle SDFs over 5×5 neighborhood.
      // Circles are separate most of the time; when noise pushes two close, smin merges them.
      { id: 'gm_sdf', type: 'noisyGridSDF', position: { x: 520, y: 260 },
        inputs: {
        ...
        outputs: { sdf: { type: 'float', label: 'SDF' } },
        params: { radius: 0.30, noiseAmt: 0.28, speed: 0.4, smoothK: 0.20, neighborRadius: 2 } },
      // Fill: hard threshold at the circle boundary (sdf < 0 = inside)
      { id: 'gm_fill', type: 'smoothstep', position: { x: 760, y: 200 },
        inputs: { value: { type: 'float', label: 'Value', connection: { nodeId: 'gm_sdf', outputKey: 'sdf' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { edge0: 0.01, edge1: -0.02 } },
      // Field vis: remap sdf to 0–1 for palette input (1 = center, 0 = outside)
      { id: 'gm_fvis', type: 'smoothstep', position: { x: 760, y: 340 },
        inputs: { value: { type: 'float', label: 'Value', connection: { nodeId: 'gm_sdf', outputKey: 'sdf' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { edge0: 0.5, edge1: -0.3 } },
      { id: 'gm_pal', type: 'palettePreset', position: { x: 760, y: 460 },
        inputs: { value: { type: 'float', label: 'Value', connection: { nodeId: 'gm_fvis', outputKey: 'result' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } },
        params: { preset: '3' } },
      { id: 'gm_col', type: 'multiplyVec3', position: { x: 1020, y: 300 },
        inputs: {
        ...
        outputs: { result: { type: 'vec3', label: 'Result' } }, params: {} },
      { id: 'gm_out', type: 'output', position: { x: 1240, y: 300 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gm_col', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Breathing — per-cell phase offset drives radius oscillation via waveRadius
  gridBreathing: {
    label: 'Grid: Breathing',
    counter: 8,
    nodes: [
      { id: 'gb_uv', type: 'uv', position: { x: 60, y: 260 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gb_grid', type: 'gridLayout', position: { x: 280, y: 260 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'gb_uv', outputKey: 'uv' } } },
        outputs: {
          cellUV:         { type: 'vec2',  label: 'Cell UV' },
          dist_to_center: { type: 'float', label: 'Dist to Center' },
          cellID:         { type: 'vec2',  label: 'Cell ID' },
        ...
        params: { columns: 10 } },
      // Compute checkerboard phase: mod(cellID.x+cellID.y, 2)*PI  → alternating 0 or π
      { id: 'gb_phase', type: 'exprNode', position: { x: 540, y: 160 },
        inputs: { cid: { type: 'vec2', label: 'cid', connection: { nodeId: 'gb_grid', outputKey: 'cellID' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'cid', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: 'mod(cid.x + cid.y, 2.0) * 3.14159', expr: 'a', label: 'phase' } },
      { id: 'gb_time', type: 'time', position: { x: 540, y: 320 },
        inputs: {}, outputs: { time: { type: 'float', label: 'Time' } }, params: {} },
      // Combine time + phase into animated distance input for waveRadius
      { id: 'gb_dist', type: 'exprNode', position: { x: 760, y: 240 },
        inputs: {
          t:   { type: 'float', label: 't',   connection: { nodeId: 'gb_time',  outputKey: 'time'   } },
          phi: { type: 'float', label: 'phi', connection: { nodeId: 'gb_phase', outputKey: 'result' } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 't', type: 'float', slider: null }, { name: 'phi', type: 'float', slider: null }],
          outputType: 'float', lines: [], result: 'sin(t * 2.0 + phi) * 0.5 + 0.5', expr: 'a', label: 'osc' } },
      { id: 'gb_draw', type: 'exprNode', position: { x: 1000, y: 260 },
        inputs: {
          cuv: { type: 'vec2',  label: 'cuv', connection: { nodeId: 'gb_grid', outputKey: 'cellUV' } },
          osc: { type: 'float', label: 'osc', connection: { nodeId: 'gb_dist', outputKey: 'result'  } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'cuv', type: 'vec2', slider: null }, { name: 'osc', type: 'float', slider: null }],
          outputType: 'float', lines: [], result: '1.0 - smoothstep(osc * 0.38, osc * 0.38 + 0.02, length(cuv))', expr: 'a', label: 'dot' } },
      { id: 'gb_pal', type: 'palettePreset', position: { x: 1000, y: 440 },
        inputs: { t: { type: 'float', label: 'T', connection: { nodeId: 'gb_dist', outputKey: 'result' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } },
        params: {} },
      { id: 'gb_col', type: 'exprNode', position: { x: 1220, y: 340 },
        inputs: {
          dot: { type: 'float', label: 'dot', connection: { nodeId: 'gb_draw', outputKey: 'result' } },
          col: { type: 'vec3',  label: 'col', connection: { nodeId: 'gb_pal',  outputKey: 'color'  } },
        },
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'dot', type: 'float', slider: null }, { name: 'col', type: 'vec3', slider: null }],
          outputType: 'vec3', lines: [], result: 'dot * col', expr: 'a', label: 'colorize' } },
      { id: 'gb_out', type: 'output', position: { x: 1440, y: 340 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gb_col', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Density Wave — gridDensityWarp animates UV before grid, producing rippling density bands
  gridDensityWave: {
    label: 'Grid: Density Wave',
    counter: 6,
    nodes: [
      { id: 'gd_uv', type: 'uv', position: { x: 60, y: 260 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gd_warp', type: 'gridDensityWarp', position: { x: 280, y: 260 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'gd_uv', outputKey: 'uv' } } },
        outputs: { warpedUV: { type: 'vec2', label: 'Warped UV' } },
        params: { amplitude: 0.08, frequency: 3.0, axis: '2.0', animated: true } },
      { id: 'gd_grid', type: 'gridLayout', position: { x: 520, y: 260 },
        ...
        params: { columns: 14 } },
      { id: 'gd_draw', type: 'exprNode', position: { x: 780, y: 260 },
        inputs: { cuv: { type: 'vec2', label: 'cuv', connection: { nodeId: 'gd_grid', outputKey: 'cellUV' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'cuv', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: '1.0 - smoothstep(0.22, 0.28, length(cuv))', expr: 'a', label: 'dot' } },
      { id: 'gd_pal', type: 'palettePreset', position: { x: 780, y: 420 },
        inputs: { t: { type: 'float', label: 'T', connection: { nodeId: 'gd_grid', outputKey: 'dist_to_center' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } },
        params: {} },
      { id: 'gd_col', type: 'exprNode', position: { x: 1020, y: 320 },
        inputs: {
        ...
        params: { inputs: [{ name: 'dot', type: 'float', slider: null }, { name: 'col', type: 'vec3', slider: null }],
          outputType: 'vec3', lines: [], result: 'dot * col', expr: 'a', label: 'colorize' } },
      { id: 'gd_out', type: 'output', position: { x: 1240, y: 320 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gd_col', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Lava Lamp — three animatedCellCenter blobs merged via sdf2dSmoothUnion
  gridLavaLamp: {
    label: 'Grid: Lava Lamp',
    counter: 9,
    nodes: [
      { id: 'll_uv', type: 'uv', position: { x: 60, y: 300 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'll_t', type: 'time', position: { x: 60, y: 460 },
        inputs: {}, outputs: { time: { type: 'float', label: 'Time' } }, params: {} },
      // Three Lissajous blob positions in g_uv space (±0.6 in x, ±0.42 in y)
      { id: 'll_p0', type: 'exprNode', position: { x: 300, y: 160 },
        inputs: { t: { type: 'float', label: 't', connection: { nodeId: 'll_t', outputKey: 'time' } } },
        outputs: { result: { type: 'vec2', label: 'Result' } },
        params: { inputs: [{ name: 't', type: 'float', slider: null }],
          outputType: 'vec2', lines: [], result: 'vec2(sin(t * 0.73) * 0.55, cos(t * 0.41) * 0.38)', expr: 'a', label: 'pos0' } },
      { id: 'll_p1', type: 'exprNode', position: { x: 300, y: 320 },
        inputs: { t: { type: 'float', label: 't', connection: { nodeId: 'll_t', outputKey: 'time' } } },
        outputs: { result: { type: 'vec2', label: 'Result' } },
        params: { inputs: [{ name: 't', type: 'float', slider: null }],
          outputType: 'vec2', lines: [], result: 'vec2(cos(t * 0.47) * 0.48, sin(t * 0.89 + 1.0) * 0.42)', expr: 'a', label: 'pos1' } },
      { id: 'll_p2', type: 'exprNode', position: { x: 300, y: 480 },
        inputs: { t: { type: 'float', label: 't', connection: { nodeId: 'll_t', outputKey: 'time' } } },
        outputs: { result: { type: 'vec2', label: 'Result' } },
        params: { inputs: [{ name: 't', type: 'float', slider: null }],
          outputType: 'vec2', lines: [], result: 'vec2(sin(t * 0.31 + 2.1) * 0.62, cos(t * 0.53 + 0.7) * 0.35)', expr: 'a', label: 'pos2' } },
      // Circle SDFs for each blob
      { id: 'll_s0', type: 'exprNode', position: { x: 580, y: 160 },
        inputs: {
          uv: { type: 'vec2', label: 'uv', connection: { nodeId: 'll_uv', outputKey: 'uv'     } },
          p:  { type: 'vec2', label: 'p',  connection: { nodeId: 'll_p0', outputKey: 'result' } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'uv', type: 'vec2', slider: null }, { name: 'p', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: 'length(uv - p) - 0.28', expr: 'a', label: 'sdf0' } },
      { id: 'll_s1', type: 'exprNode', position: { x: 580, y: 320 },
        inputs: {
          uv: { type: 'vec2', label: 'uv', connection: { nodeId: 'll_uv', outputKey: 'uv'     } },
          p:  { type: 'vec2', label: 'p',  connection: { nodeId: 'll_p1', outputKey: 'result' } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'uv', type: 'vec2', slider: null }, { name: 'p', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: 'length(uv - p) - 0.24', expr: 'a', label: 'sdf1' } },
      { id: 'll_s2', type: 'exprNode', position: { x: 580, y: 480 },
        inputs: {
          uv: { type: 'vec2', label: 'uv', connection: { nodeId: 'll_uv', outputKey: 'uv'     } },
          p:  { type: 'vec2', label: 'p',  connection: { nodeId: 'll_p2', outputKey: 'result' } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'uv', type: 'vec2', slider: null }, { name: 'p', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: 'length(uv - p) - 0.22', expr: 'a', label: 'sdf2' } },
      // Smooth union chain: (s0 ∪ s1) ∪ s2
      { id: 'll_u01', type: 'sdf2dSmoothUnion', position: { x: 840, y: 240 },
        inputs: {
          sdfA: { type: 'float', label: 'SDF A', connection: { nodeId: 'll_s0', outputKey: 'result' } },
          sdfB: { type: 'float', label: 'SDF B', connection: { nodeId: 'll_s1', outputKey: 'result' } },
        },
        outputs: { result: { type: 'float', label: 'Result' }, blend: { type: 'float', label: 'Blend' } },
        params: { k: 0.18 } },
      { id: 'll_u012', type: 'sdf2dSmoothUnion', position: { x: 840, y: 440 },
        inputs: {
          sdfA: { type: 'float', label: 'SDF A', connection: { nodeId: 'll_u01', outputKey: 'result' } },
          sdfB: { type: 'float', label: 'SDF B', connection: { nodeId: 'll_s2',  outputKey: 'result' } },
        },
        outputs: { result: { type: 'float', label: 'Result' }, blend: { type: 'float', label: 'Blend' } },
        params: { k: 0.2 } },
      { id: 'll_fill', type: 'exprNode', position: { x: 1080, y: 340 },
        inputs: {
          sdf:   { type: 'float', label: 'sdf',   connection: { nodeId: 'll_u012', outputKey: 'result' } },
          blend: { type: 'float', label: 'blend', connection: { nodeId: 'll_u012', outputKey: 'blend'  } },
        },
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'sdf', type: 'float', slider: null }, { name: 'blend', type: 'float', slider: null }],
          outputType: 'vec3', lines: [], result: 'smoothstep(0.015, -0.015, sdf) * mix(vec3(1.0,0.35,0.05), vec3(0.05,0.55,1.0), blend)', expr: 'a', label: 'fill' } },
      { id: 'll_out', type: 'output', position: { x: 1320, y: 340 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'll_fill', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Onion Rings — circleSDF repeatedly onion-shelled for concentric ring structure
  gridOnionRings: {
    label: 'Grid: Onion Rings',
    counter: 8,
    nodes: [
      { id: 'go_uv', type: 'uv', position: { x: 60, y: 280 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'go_grid', type: 'gridLayout', position: { x: 280, y: 280 },
      ...
      { id: 'go_draw', type: 'exprNode', position: { x: 980, y: 340 },
        ...
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'sdf', type: 'float', slider: null }, { name: 'dtc', type: 'float', slider: null }],
          outputType: 'vec3', lines: [], result: 'smoothstep(0.015, -0.015, sdf) * mix(vec3(0.0,0.8,1.0), vec3(1.0,0.2,0.5), dtc * 2.0)', expr: 'a', label: 'rings' } },
      { id: 'go_out', type: 'output', position: { x: 1220, y: 340 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'go_draw', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Iron Filings — atan angle from each cell center to mouse drives sdBox orientation via uvTransform2d
  gridIronFilings: {
    label: 'Grid: Iron Filings',
    counter: 9,
    nodes: [
      { id: 'gi_uv', type: 'uv', position: { x: 60, y: 280 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gi_mouse', type: 'mouse', position: { x: 60, y: 440 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gi_grid', type: 'gridLayout', position: { x: 280, y: 280 },
        ...
        params: { columns: 12 } },
      // Angle from cell center to mouse — aligns each filing toward the attractor
      { id: 'gi_angle', type: 'exprNode', position: { x: 540, y: 360 },
        inputs: {
        ...
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'cc', type: 'vec2', slider: null }, { name: 'mouse', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: 'atan(mouse.y - cc.y, mouse.x - cc.x)', expr: 'a', label: 'angle' } },
      // Rotate cell UV by that angle, then draw the filing bar
      { id: 'gi_txfm', type: 'uvTransform2d', position: { x: 800, y: 280 },
        inputs: {
        ...
        outputs: { result: { type: 'vec2', label: 'Result' } },
        params: { tx: 0.0, ty: 0.0, sx: 1.0, sy: 1.0, px: 0.0, py: 0.0 } },
      { id: 'gi_box', type: 'sdBox', position: { x: 1040, y: 280 },
        inputs: { p: { type: 'vec2', label: 'P', connection: { nodeId: 'gi_txfm', outputKey: 'result' } } },
        outputs: { distance: { type: 'float', label: 'Distance' } },
        params: { bx: 0.16, by: 0.025 } },
      // Color: warm gold tinted by proximity to mouse (attractAmount via cell displace idea reused as dist)
      { id: 'gi_draw', type: 'exprNode', position: { x: 1280, y: 280 },
        inputs: {
        ...
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'sdf', type: 'float', slider: null }, { name: 'cc', type: 'vec2', slider: null }, { name: 'm', type: 'vec2', slider: null }],
          outputType: 'vec3', lines: [], result: 'smoothstep(0.01, -0.01, sdf) * mix(vec3(0.55,0.45,0.25), vec3(1.0,0.9,0.5), smoothstep(1.5, 0.0, length(m - cc)))', expr: 'a', label: 'filing' } },
      { id: 'gi_out', type: 'output', position: { x: 1500, y: 280 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gi_draw', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Frequency Columns — per-column sin offset drives bar height for a spectrum visualizer feel
  gridFreqColumns: {
    label: 'Grid: Freq Columns',
    counter: 7,
    nodes: [
      { id: 'gf_uv', type: 'uv', position: { x: 60, y: 260 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'gf_grid', type: 'gridLayout', position: { x: 280, y: 260 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'gf_uv', outputKey: 'uv' } } },
        outputs: {
          cellUV:         { type: 'vec2',  label: 'Cell UV' },
          dist_to_center: { type: 'float', label: 'Dist to Center' },
          cellID:         { type: 'vec2',  label: 'Cell ID' },
        ...
        params: { columns: 18 } },
      // Per-column animated bar height: sin with column-seeded frequency
      { id: 'gf_barH', type: 'exprNode', position: { x: 540, y: 200 },
        inputs: { cid: { type: 'vec2', label: 'cid', connection: { nodeId: 'gf_grid', outputKey: 'cellID' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'cid', type: 'vec2', slider: null }],
          outputType: 'float', lines: [], result: '0.3 + 0.6 * abs(sin(u_time * (0.8 + cid.x * 0.19) + cid.x * 1.3))', expr: 'a', label: 'barH' } },
      // Draw column: cell is lit if its cellUV.y (bottom-of-cell = -0.5) is within bar height
      { id: 'gf_draw', type: 'exprNode', position: { x: 780, y: 260 },
        inputs: {
          cuv: { type: 'vec2',  label: 'cuv', connection: { nodeId: 'gf_grid', outputKey: 'cellUV' } },
          h:   { type: 'float', label: 'h',   connection: { nodeId: 'gf_barH', outputKey: 'result' } },
        },
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'cuv', type: 'vec2', slider: null }, { name: 'h', type: 'float', slider: null }],
          outputType: 'float', lines: [], result: 'step(cuv.y + 0.5, h) * step(abs(cuv.x) - 0.42, 0.0)', expr: 'a', label: 'bar' } },
      { id: 'gf_pal', type: 'palettePreset', position: { x: 780, y: 420 },
        inputs: { t: { type: 'float', label: 'T', connection: { nodeId: 'gf_barH', outputKey: 'result' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } },
        params: {} },
      { id: 'gf_col', type: 'exprNode', position: { x: 1020, y: 320 },
        inputs: {
          bar: { type: 'float', label: 'bar', connection: { nodeId: 'gf_draw', outputKey: 'result' } },
          col: { type: 'vec3',  label: 'col', connection: { nodeId: 'gf_pal',  outputKey: 'color'  } },
        },
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'bar', type: 'float', slider: null }, { name: 'col', type: 'vec3', slider: null }],
          outputType: 'vec3', lines: [], result: 'bar * col', expr: 'a', label: 'colorize' } },
      { id: 'gf_out', type: 'output', position: { x: 1240, y: 320 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'gf_col', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },

  // Grid: Scatter — hash-displaced colored dots; NeighborDist prevents boundary clipping
  gridNeighborDisplaced: {
    label: 'Grid: Scatter',
    counter: 7,
    nodes: [
      { id: 'nd2_uv0', type: 'uv', position: { x: 60, y: 200 },
        inputs: {}, outputs: { uv: { type: 'vec2', label: 'UV' } }, params: {} },
      { id: 'nd2_grid1', type: 'gridLayout', position: { x: 280, y: 200 },
        inputs: { uv: { type: 'vec2', label: 'UV', connection: { nodeId: 'nd2_uv0', outputKey: 'uv' } } },
        outputs: {
          cellUV:         { type: 'vec2',  label: 'Cell UV' },
          dist_to_center: { type: 'float', label: 'Dist to Center' },
          cellID:         { type: 'vec2',  label: 'Cell ID' },
      ...
      { id: 'nd2_mask3', type: 'exprNode', position: { x: 760, y: 140 },
        ...
        outputs: { result: { type: 'float', label: 'Result' } },
        params: { inputs: [{ name: 'd', type: 'float', slider: null }],
          outputType: 'float', lines: [], result: '1.0 - smoothstep(0.28, 0.36, d)',
          expr: 'a', label: 'soft circle' } },
      { id: 'nd2_pal4', type: 'palettePreset', position: { x: 760, y: 300 },
        inputs: { t: { type: 'float', label: 'T', connection: { nodeId: 'nd2_grid1', outputKey: 'dist_to_center' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } }, params: {} },
      { id: 'nd2_mul5', type: 'exprNode', position: { x: 1000, y: 200 },
        inputs: {
          mask: { type: 'float', label: 'mask', connection: { nodeId: 'nd2_mask3', outputKey: 'result' } },
          col:  { type: 'vec3',  label: 'col',  connection: { nodeId: 'nd2_pal4',  outputKey: 'color'  } },
        },
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'mask', type: 'float', slider: null }, { name: 'col', type: 'vec3', slider: null }],
          outputType: 'vec3', lines: [], result: 'mask * col', expr: 'a', label: 'colorize' } },
      { id: 'nd2_out6', type: 'output', position: { x: 1220, y: 200 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'nd2_mul5', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },
...
// ── Example folders ───────────────────────────────────────────────────────────
// Groups EXAMPLE_GRAPHS keys into categories for the desktop NodePalette's
// Examples browser and the mobile examples picker (App.tsx) — one taxonomy,
// shared, so the two surfaces never drift.
