  ...
  gridNeighborDisplaced: {
    ...
    nodes: [
      ...
      { id: 'nd2_grid1', type: 'gridLayout', position: { x: 280, y: 200 },
        ...
        params: { columns: 9 } },
      { id: 'nd2_hash2', type: 'exprNode', position: { x: 520, y: 340 },
        inputs: { cid: { type: 'vec2', label: 'cid', connection: { nodeId: 'nd2_grid1', outputKey: 'cellID' } } },
        outputs: { result: { type: 'vec2', label: 'Result' } },
        params: { inputs: [{ name: 'cid', type: 'vec2', slider: null }],
          outputType: 'vec2', lines: [],
          result: '(fract(sin(vec2(dot(cid, vec2(127.1, 311.7)), dot(cid, vec2(269.5, 183.3)))) * 43758.5453) - 0.5) * 0.44',
          expr: 'a', label: 'cell hash' } },
      { id: 'nd2_nd3', type: 'neighborDist', position: { x: 520, y: 140 },
        inputs: {
          cellUV:       { type: 'vec2', label: 'Cell UV',      connection: { nodeId: 'nd2_grid1', outputKey: 'cellUV'  } },
          displacement: { type: 'vec2', label: 'Displacement', connection: { nodeId: 'nd2_hash2', outputKey: 'result'  } },
        },
        outputs: { minDist: { type: 'float', label: 'Min Dist' } },
        params: { neighborhood_size: 1, initial_dist: 999.0 } },
      { id: 'nd2_mask4', type: 'exprNode', position: { x: 760, y: 140 },
        inputs: { d: { type: 'float', label: 'd', connection: { nodeId: 'nd2_nd3', outputKey: 'minDist' } } },
        outputs: { result: { type: 'float', label: 'Result' } },
      ...
      { id: 'nd2_pal5', type: 'palettePreset', position: { x: 760, y: 300 },
        inputs: { t: { type: 'float', label: 'T', connection: { nodeId: 'nd2_grid1', outputKey: 'dist_to_center' } } },
        outputs: { color: { type: 'vec3', label: 'Color' } }, params: {} },
      { id: 'nd2_mul6', type: 'exprNode', position: { x: 1000, y: 200 },
        inputs: {
          mask: { type: 'float', label: 'mask', connection: { nodeId: 'nd2_mask4', outputKey: 'result' } },
          col:  { type: 'vec3',  label: 'col',  connection: { nodeId: 'nd2_pal5',  outputKey: 'color'  } },
        },
        outputs: { result: { type: 'vec3', label: 'Result' } },
        params: { inputs: [{ name: 'mask', type: 'float', slider: null }, { name: 'col', type: 'vec3', slider: null }],
          outputType: 'vec3', lines: [], result: 'mask * col', expr: 'a', label: 'colorize' } },
      { id: 'nd2_out7', type: 'output', position: { x: 1220, y: 200 },
        inputs: { color: { type: 'vec3', label: 'Color', connection: { nodeId: 'nd2_mul6', outputKey: 'result' } } },
        outputs: {}, params: {} },
    ],
  },
