...
export const EXAMPLE_INDEX: Record<string, { label: string; description?: string }> = {
  ...
  fcSolar: { label: "FC: Solar", description: "After \"solar\" on FragCoord (community shader; author credit to follow). Circle SDF \u2192 1/max(\u2212d, 10d) glow, dimmed by a per-pixel flicker and slow pulse, tinted, Tone Map Tanh\u00b2 for the golfed-shader roll-off." },
  fcPillars: { label: "FC: Pillars", description: "After \"pillars\" on FragCoord (community shader; author credit to follow). Scrolling UV \u2192 Infinite Repeat (2\u00d72 cells) \u2192 a sqrt(1 \u2212 x\u00b2) cylinder profile that flips sign per column \u2192 sine Palette." },
  fcGradient4: { label: "FC: Gradient 4", description: "After \"gradient 4\" on FragCoord (community shader; author credit to follow). One phase expression (folded cosines + hash grain) into a sine Palette, divided by |cos(x/0.1)| stripes. Noise Float in Hash mode replaces the fract(cos(dot\u2026)) one-liner." },
  fcGrainGradient: { label: "FC: Grain Gradient", description: "After \"grain gradient\" on FragCoord (community shader; author credit to follow). Perlin Noise Float \u2192 Remap \u2192 Rotate 2D spins the canvas; a two-line wave warp; four Mix nodes cycle six colours; two Smoothstep-driven Mix layers; Brightness/Contrast and Grain finish." },
  fcTiling: { label: "FC: Rotating Cross Tiles", description: "After \"tiling\" on FragCoord (community shader; author credit to follow). One group used twice: Rotate 2D \u2192 Infinite Repeat (\u221a10 cells) \u2192 un-rotate \u2192 per-cell spin phase from the Cell ID \u2192 Shape SDF Cross \u2192 Smoothstep. The white and black grids are offset copies; an expression picks whichever is spinning." },
  fcTheScreen: { label: "FC: The Screen", description: "After \"the screen\" on FragCoord (community shader; author credit to follow). A 16-iteration group (the loop cap; the original steps ~127 times): Loop Index \u2192 layer depth \u2192 one projection expression \u2192 window-light expression accumulated with +=. Then a \u2074\u221a tone tail. Shows how a golfed for-loop maps onto an iterated group." },
  fcShield: { label: "FC: Shield", description: "After \"Shield\" on FragCoord (community shader; author credit to follow). 16 zoomed layers in an iterated group (the loop cap; the original uses 100): Spherical (Dome mode) bulges each layer and its Height output shades it; Infinite Repeat with Stagger 0.5 makes the brickwork; an edge-glow expression accumulates; Tone Map Tanh\u00b2." },
  fcMainFrame: { label: "FC: Main Frame", description: "After \"main frame\" on FragCoord (community shader; author credit to follow). Nine layers in an iterated group; each runs a 7-step fold in a Custom Function, then 0.02/l\u00b2 glow \u00d7 a cosine Palette accumulates. Previous Frame sampled through a sine warp, squared and added, gives the feedback smear; Tone Map Tanh." },
  fcAtlantic: { label: "FC: Atlantic", description: "After \"atlantic\" on FragCoord (community shader; author credit to follow). March Camera + Scene Group + March Loop Group. The scene is Scene Pos \u2192 Turbulence 3D (ten sine octaves) \u2192 tilted plane \u2192 a soft step-size expression; the volumetric loop accumulates 1.2/(d\u00b7z); tint and Tone Map Tanh." },
  fcOrb: { label: "FC: Orb", description: "After \"orb\" on FragCoord (community shader; author credit to follow). March Camera orbiting a Scene Group: a gyroid shell (|1.4 + cos\u00b7cos|) intersected with a Sphere 3D. Inside the volumetric March Loop Group a Palette of the x position, divided by the distance, accumulates per step; Tone Map Tanh." },
  fcBitshift: { label: "FC: Bitshift", description: "After \"bitshift\" on FragCoord (community shader; author credit to follow). Split \u2192 two Quantize (1/32) \u2192 Make Vec2 snaps the UV to a coarse grid; one expression for the tan() ripple; \u00d7\u00bc then Posterize (4 levels) reproduces floor(x)/4." },
  fcTrippyNoise: { label: "FC: Trippy Noise", description: "After \"trippy noise\" on FragCoord (community shader; author credit to follow). Rotate 2D \u2192 |uv| \u2192 Polar angle drives a second Rotate; three Noise Floats (value noise, offset \u00b10.333) \u2192 Smoothstep \u2192 RGB tints summed; Vignette, lift, Texture Input overlay via Blend Modes, and Bloom replaces the threshold \u2192 blur X \u2192 blur Y \u2192 screen passes." },
  comboRepeatCellHash: { label: "Combo: Repeat + Cell ID + Hash", description: "Infinite Repeat (Stagger 0.5) tiles the UV; its Cell ID feeds a Hash Noise Float \u2192 Palette so every brick gets its own colour; Circle SDF on the Cell UV \u2192 SDF Fill. The standard repeat-and-vary recipe." },
  comboTurbulenceGlow: { label: "Combo: Turbulence + SDF + Glow", description: "Turbulence (Xor's sine loop) warps the UV before a Circle SDF; SDF Glow in Simple mode with a Palette tint turns the wobbling distance into light. Swap the SDF for FBM or a Grid to see the warp on anything." },
  comboBloomDots: { label: "Combo: Grid + SDF Fill + Bloom", description: "Grid \u2192 Circle SDF on the Cell UV \u2192 SDF Fill draws bright dots coloured by Cell ID; Bloom (Luma select, Layered kernel \u2014 Xor's bloom article) thresholds, blurs and screens the highlights: the whole threshold \u2192 Blur X \u2192 Blur Y \u2192 screen pass chain in one node." },
  comboDomeRepeat: { label: "Combo: Dome + Repeat + Height", description: "Spherical in Dome mode bulges the plane like a hemisphere; Infinite Repeat tiles a Box SDF over it; the Height output shades the dome and masks everything outside the unit circle." },
  comboChaosStars: { label: "Combo: Chaos Layers + Glow to Color", description: "Chaos Layers (Xor's Efficient Chaos: five golden-angle rotated, shifted, scaled cell grids with parallax) makes a starfield; Glow to Color tints the summed light and its Layer output colours near stars warmer via a Palette." },
  comboBlurDirectional: { label: "Combo: Wave Texture + Blur H/V", description: "A sharp Wave Texture through Gaussian Blur set to Horizontal only: the separable Blur X pass from the blur articles, on its own a streak. Switch Direction to Vertical or Both on the card to compare." },
};
...
export const EXAMPLE_FOLDERS: Array<{ label: string; color: string; keys: string[] }> = [
  ...
  { label: "Color & Lighting",  color: ctp.peach, keys: ['glowCircle','blackbodyDemo','blendModesDemo','toneMapDemo','angularGradient','shapeShowcase','fbmLandscape','spectralLens','vec3SwizzlePalette','vec2SwizzleUV','colorRampFBM','oklabGradient','neonGlow'] },
  ...
  { label: "FragCoord Picks",    color: ctp.yellow, keys: ['fcTrippyNoise','fcSolar','fcPillars','fcGradient4','fcGrainGradient','fcTiling','fcTheScreen','fcShield','fcMainFrame','fcAtlantic','fcOrb','fcBitshift'] },
  { label: "Node Combos",        color: ctp.flamingo, keys: ['comboChaosStars','comboRepeatCellHash','comboTurbulenceGlow','comboBloomDots','comboDomeRepeat'] },
];
