...
export const EXAMPLE_INDEX: Record<string, { label: string; description?: string }> = {
  ...
  comboDomeRepeat: { label: "Combo: Dome + Repeat + Height", description: "Spherical in Dome mode bulges the plane like a hemisphere; Infinite Repeat tiles a Box SDF over it; the Height output shades the dome and masks everything outside the unit circle. Dome mode where it came from: FC: Shield; the Repeat + Cell ID half: Combo: Repeat + Cell ID + Hash." },
  ...
  comboBlurDirectional: { label: "Combo: Wave Texture + Blur H/V", description: "A sharp Wave Texture through Gaussian Blur set to Horizontal only: the separable Blur X pass from the blur articles, on its own a streak. Switch Direction to Vertical or Both on the card to compare. Both passes plus threshold and screen in one node: Combo: Grid + SDF Fill + Bloom." },
};
