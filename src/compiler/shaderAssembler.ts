...
export class ShaderAssembler {
  ...
  private compileNode(node: GraphNode): void {
    ...
    if (node.type === 'group') { this.compileGroupNode(node, inputVars, nodeSlug); return; }
    if (node.type === 'sceneGroup') { this.compileSceneGroupNode(node, inputVars, nodeSlug); return; }
  ...
  private compileGroupNode(node: GraphNode, inputVars: Record<string, string>, nodeSlug: string): void {
              ...
              if (subNode.type === 'group') {
                ...
                for (const [k, inp] of Object.entries(subNode.inputs)) {
                  ...
                  if (portInputOverrides.has(portKey)) {
                    ...
                    const srcOutputs = this.nodeOutputs.get(inp.connection.nodeId);
                ...
                for (const inn of innerSubgraph.nodes) {
                  if (inn.type === 'loopIndex') {
                    ...
                    this.nodeOutputs.set(innerPrefix + innSlug, { i: '0.0' });
                  }
                }
                ...
                for (const port of innerSubgraph.outputPorts) {
                  ...
                  const fromOut = this.nodeOutputs.get(innerPrefix + mappedFromId);
          ...
          if (iters <= 1) {
            ...
            for (const port of (subgraph.outputPorts ?? [])) {
              ...
              const fromOutputs = this.nodeOutputs.get(prefix + mappedFromNodeId);
              if (fromOutputs?.[port.fromOutputKey]) groupOutputVars[port.key] = fromOutputs[port.fromOutputKey];
            }
            this.nodeOutputs.set(node.id, groupOutputVars);
          } else {
            ...
            for (const { inPort } of carryPairs) {
              ...
              this.mainCode.push(`    ${inPort.type} ${varName} = ${initVal};\n`);
            }
            ...
            for (const port of outPorts) {
              if (!carryOutKeys.has(port.key)) {
                ...
                this.mainCode.push(`    ${port.type} ${varName} = ${defaultGlslVal(port.type as DataType)};\n`);
              }
            }
            ...
            for (const sn of subgraph.nodes) {
              ...
              if (portInputOverrides.has(portKey)) {
                ...
                if (conn) {
                  ...
                  const srcOut = this.nodeOutputs.get(conn.nodeId)?.[conn.outputKey]
                              ?? this.nodeOutputs.get(prefix + connSlug)?.[conn.outputKey];
              ...
              this.mainCode.push(`    ${actualOutType} ${naturalVarName};\n`);
              this.mainCode.push(`    ${naturalVarName} = ${initVar};\n`);
            ...
            for (const sn of subgraph.nodes) {
              ...
              for (const [outKey, outSock] of Object.entries(def.outputs)) {
                ...
                this.mainCode.push(`    ${actualType} ${varName} = ${initExpr};\n`);
              }
            ...
            for (const sn of loopCarryNodes) {
              ...
              if (initConn) {
                ...
                const srcOut = this.nodeOutputs.get(prefix + initConnSlug)?.[initConn.outputKey];
                if (srcOut) initVar = srcOut;
              ...
              this.mainCode.push(`    ${carryType} ${carryVarName} = ${initVar};\n`);
              // Pre-inject so compileSubgraphPass sees this node as already done
              this.nodeOutputs.set(prefix + snSlugLc, { value: carryVarName });
            }
            ...
            for (const sn of subgraph.nodes) {
              ...
              for (const inp of carryInputs) {
                ...
                if (initConn) {
                  ...
                  const srcOut = this.nodeOutputs.get(prefix + initSlug)?.[initConn.outputKey]
                              ?? this.nodeOutputs.get(initConn.nodeId)?.[initConn.outputKey];
                  if (srcOut) initVar = srcOut;
                }
                if (initVar === defaultGlslVal(inp.type)) {
                  ...
                  if (conn) {
                    const connSlug = subSlugMap.get(conn.nodeId) ?? conn.nodeId;
                    const srcOut = this.nodeOutputs.get(prefix + connSlug)?.[conn.outputKey]
                                ?? this.nodeOutputs.get(conn.nodeId)?.[conn.outputKey];
                    if (srcOut) initVar = srcOut;
                  }
                }
                ...
                this.mainCode.push(`    ${inp.type} ${carryVarName} = ${initVar};\n`);
              }
            ...
            for (const sn of loopCarryNodes) {
              ...
              if (nextConn) {
                ...
                const nextVar = this.nodeOutputs.get(prefix + nextConnSlug)?.[nextConn.outputKey];
                if (nextVar) {
                  this.mainCode.push(`    ${loopCarryVarNames[sn.id]} = ${nextVar};\n`);
                }
              }
            }

            // 3c. Apply assignOp accumulations for nodes marked with += / -= / *= / /=
            //     Emit `outerVar OP innerVar;` then update this.nodeOutputs to point to the outer var.
            ...
            for (const sn of subgraph.nodes) {
              ...
              const freshOutputs = this.nodeOutputs.get(prefix + snSlugAcc2);
              ...
              for (const [outKey, outerVarName] of Object.entries(outVars)) {
                ...
                if (freshVar) {
                  this.mainCode.push(`    ${outerVarName} ${op} ${freshVar};\n`);
                }
                // Point this.nodeOutputs to the outer accumulated var for downstream reads
                updatedOutputs[outKey] = outerVarName;
              }
              this.nodeOutputs.set(prefix + snSlugAcc2, updatedOutputs);
            }
            ...
            for (const { inPort, outPort } of carryPairs) {
              ...
              if (fromOutputs?.[outPort.fromOutputKey]) {
                this.mainCode.push(`    ${carryVarNames[inPort.key]} = ${fromOutputs[outPort.fromOutputKey]};\n`);
              }
            }
            ...
            for (const port of outPorts) {
              if (!carryOutKeys.has(port.key)) {
                ...
                if (innerVar) {
                  this.mainCode.push(`    ${nonCarryOutVarNames[port.key]} = ${innerVar};\n`);
                }
              }
            }
  ...
  private compileSceneGroupNode(node: GraphNode, inputVars: Record<string, string>, nodeSlug: string): void {
          const subgraph = node.params.subgraph as SubgraphData | undefined;
          if (!subgraph || subgraph.nodes.length === 0) {
            this.nodeOutputs.set(node.id, { scene: 'MISSING_SCENE' });
            return;
          }
          ...
          for (const sn of subgraph.nodes) {
            sgSubSlugMap.set(sn.id, computeNodeSlug(sn, this.usedSlugs));
          }
          ...
          for (const sn of subgraph.nodes) {
            if (sn.type === 'scenePos') {
              ...
              this.nodeOutputs.set(sgPrefix + spSlug, { pos: 'p' });
            }
          }
          ...
          for (const sn of sgSorted) {
            ...
            if (sn.type === 'group') {
              ...
              if (!grpSubgraph || grpSubgraph.nodes.length === 0) { this.nodeOutputs.set(sn.id, {}); continue; }
              ...
              const grpPrefixed: GraphNode[] = grpSubgraph.nodes.map(gn => {
                ...
                if (gnDefForOverrides?.paramDefs) {
                  for (const paramKey of Object.keys(gnDefForOverrides.paramDefs)) {
                    ...
                    if (psInp?.connection) {
                      const srcVar = this.nodeOutputs.get(psInp.connection.nodeId)?.[psInp.connection.outputKey];
              ...
              for (const port of (grpSubgraph.outputPorts ?? [])) {
                const s = this.nodeOutputs.get(grpPrefix + (grpSlugMap.get(port.fromNodeId) ?? port.fromNodeId));
                if (s?.[port.fromOutputKey]) {
                  grpOutVars[port.key] = s[port.fromOutputKey];
                  if (port.type === 'float') sgLastFloatVar = s[port.fromOutputKey];
                }
              }
              this.nodeOutputs.set(sn.id, grpOutVars);
              continue;
            }
          ...
          for (const [sid, sslug] of sgSubSlugMap) this.nodeSlugMap.set(sid, sgPrefix + sslug);
          ...
          for (const port of (subgraph.outputPorts ?? [])) {
            if (port.type === 'float') {
              ...
              const srcOut = this.nodeOutputs.get(sgPrefix + mappedFromNodeId);
  ...
  private compileMarchLoopGroupNode(node: GraphNode, inputVars: Record<string, string>, nodeSlug: string): void {
          ...
          if (subgraph && subgraph.nodes.length > 0) {
            ...
            for (const sn of subgraph.nodes) {
              mlSubSlugMap.set(sn.id, computeNodeSlug(sn, this.usedSlugs));
            }
            ...
            for (const sn of subgraph.nodes) {
              if (sn.type === 'marchPos') {
                ...
                this.nodeOutputs.set(mlPrefix + slug, { pos: `${nodeSlug}_mp` });
              }
              if (sn.type === 'marchDist') {
                const slug = mlSubSlugMap.get(sn.id) ?? sn.id;
                this.nodeOutputs.set(mlPrefix + slug, { dist: `${nodeSlug}_bt`, t: `${nodeSlug}_bt` });
              }
              if (sn.type === 'marchLoopInputs') {
                ...
                this.nodeOutputs.set(mlPrefix + slug, {
                  ro:        `${nodeSlug}_ro`,
                  rd:        `${nodeSlug}_rd`,
            ...
            for (const sn of mlProcessOrder) {
              ...
              if (sn.type === 'time') {
                this.nodeOutputs.set(sn.id, { time: 'u_time' });
                continue;
              }
              if (sn.type === 'mousePos' || sn.type === 'mouse') {
                ...
                this.nodeOutputs.set(sn.id, { mouse: `${mId}_uv`, xy: `${mId}_uv`, x: `${mId}_x`, y: `${mId}_y` });
                continue;
              }
              ...
              if (sn.type === 'sceneGroup') {
                ...
                if (sgSubgraph && sgSubgraph.nodes.length > 0) {
                  ...
                  for (const inn of sgSubgraph.nodes) {
                    sgInnerSlugMap.set(inn.id, computeNodeSlug(inn, this.usedSlugs));
                  }
                  ...
                  for (const inn of sgSubgraph.nodes) {
                    if (inn.type === 'scenePos') {
                      ...
                      this.nodeOutputs.set(sgInnerPrefix + spSlug, { pos: 'p' });
                    }
                  }
                  ...
                  for (const sgn of sgInnerSorted) {
                    if (this.nodeOutputs.has(sgn.id)) continue; // scenePos already registered

                    // Inline nested group nodes inside the scene function
                    if (sgn.type === 'group') {
                      const igrpSub = sgn.params.subgraph as SubgraphData | undefined;
                      if (!igrpSub || igrpSub.nodes.length === 0) { this.nodeOutputs.set(sgn.id, {}); continue; }
                      ...
                      for (const port of (igrpSub.outputPorts ?? [])) {
                        const s = this.nodeOutputs.get(igrpPrefix + (igrpSlugMap.get(port.fromNodeId) ?? port.fromNodeId));
                  ...
                  for (const port of (sgSubgraph.outputPorts ?? [])) {
                    if (port.type === 'float') {
                      ...
                      const srcOut = this.nodeOutputs.get(sgInnerPrefix + mappedFromId);
                      if (srcOut?.[port.fromOutputKey]) sgLastFloatVar = srcOut[port.fromOutputKey];
                    }
                  }
                  ...
                  this.functions.add(sgFnDef);
                  if (sgInlineExtraParams.length > 0) this.sceneFnExtraParams.set(sceneFnNameLocal, sgInlineExtraParams);
                  ...
                  this.nodeOutputs.set(sn.id, { scene: sceneFnNameLocal });
                }
              ...
              if (sn.type === 'group') {
                ...
                if (!mlGrpSubgraph || mlGrpSubgraph.nodes.length === 0) { this.nodeOutputs.set(sn.id, {}); continue; }
                const mlGrpSlugMap = new Map<string, string>();
                for (const gn of mlGrpSubgraph.nodes) mlGrpSlugMap.set(gn.id, computeNodeSlug(gn, this.usedSlugs));
                ...
                for (const gn of topologicalSort(mlGrpPrefixed)) {
                  ...
                  if (gn.type === 'marchSceneDist') {
                    ...
                    if (!resolvedSceneFn2) {
                      ...
                      if (mlVolumetric) {
                        ...
                        this.nodeOutputs.set(gn.id, { dist: outVar2, rawDist: rawVar2 });
                      } else {
                        bodyLines.push(`    float ${outVar2} = ${resolvedSceneFn2}(${posVar2}${resolvedExtraStr2});\n`);
                        this.nodeOutputs.set(gn.id, { dist: outVar2 });
                      }
                    }
                    continue;
                  }
                  ...
                  if (origGnForAcc?.assignOp && origGnForAcc.assignOp !== '=') {
                    ...
                    this.nodeOutputs.set(gn.id, accOutVars);
                  } else {
                    this.nodeOutputs.set(gn.id, gnResult.outputVars);
                ...
                for (const port of (mlGrpSubgraph.outputPorts ?? [])) {
                  const s = this.nodeOutputs.get(mlGrpPrefix + (mlGrpSlugMap.get(port.fromNodeId) ?? port.fromNodeId));
                  if (s?.[port.fromOutputKey]) {
                    mlGrpOutVars[port.key] = s[port.fromOutputKey];
                    if (port.type === 'vec3') mlLastVec3Var = s[port.fromOutputKey];
                  }
                }
                this.nodeOutputs.set(sn.id, mlGrpOutVars); continue;
              }
              ...
              if (sn.type === 'marchSceneDist') {
                ...
                if (!resolvedSceneFn) {
                  ...
                  if (mlVolumetric) {
                    ...
                    this.nodeOutputs.set(sn.id, { dist: outVar, rawDist: rawVar });
                  } else {
                    bodyLines.push(`    float ${outVar} = ${resolvedSceneFn}(${posVar}${resolvedExtraStr});\n`);
                    this.nodeOutputs.set(sn.id, { dist: outVar });
                  }
                }
                continue;
              }
              ...
              if (sn.assignOp && sn.assignOp !== '=') {
                ...
                this.nodeOutputs.set(sn.id, accOutVars);
                continue;
              }
            ...
            // Populate this.nodeSlugMap for march loop group body nodes
            for (const [sid, sslug] of mlSubSlugMap) this.nodeSlugMap.set(sid, mlPrefix + sslug);
            ...
            for (const port of (subgraph.outputPorts ?? [])) {
              if (port.type === 'vec3') {
                ...
                const srcOut = this.nodeOutputs.get(mlPrefix + mappedFromNodeId);
          ...
          if (Object.keys(mlgAccSockets).length > 0) this.mlgDynamicOutputs.set(node.id, mlgAccSockets);
          this.nodeOutputs.set(node.id, {
            ...
            for (const sn of mlProcessOrder) {
              ...
              if (sn.type === 'sceneGroup') {
                ...
                if (sgSubgraph && sgSubgraph.nodes.length > 0) {
                  ...
                  for (const sgn of sgInnerSorted) {
                    if (this.nodeOutputs.has(sgn.id)) continue;
  ...
  private compileSpaceWarpGroupNode(node: GraphNode, inputVars: Record<string, string>, nodeSlug: string): void {
          const subgraph = node.params.subgraph as SubgraphData | undefined;
          if (!subgraph || subgraph.nodes.length === 0) {
            this.nodeOutputs.set(node.id, { warp: '' });
            return;
          }
          ...
          for (const sn of subgraph.nodes) {
            swSubSlugMap.set(sn.id, computeNodeSlug(sn, this.usedSlugs));
          }
          ...
          for (const sn of subgraph.nodes) {
            if (sn.type === 'scenePos') {
              ...
              this.nodeOutputs.set(swPrefix + spSlug, { pos: 'p' });
            }
          }
          ...
          for (const port of (subgraph.outputPorts ?? [])) {
            if (port.type === 'vec3') {
              ...
              const srcOut = this.nodeOutputs.get(swPrefix + mappedFromNodeId);
          ...
          this.nodeOutputs.set(node.id, { warp: fnName });
          return;

  }

  private compileBypassNode(node: GraphNode, inputVars: Record<string, string>, nodeSlug: string, def: NonNullable<ReturnType<typeof getNodeDefinition>>): void {
        ...
        if (override) {
          ...
          if (node.assignOp && node.assignOp !== '=') {
            ...
            for (const outKey of Object.keys(result.outputVars)) {
              ...
              this.mainCode.push(`    ${actualType} ${accVar} = ${initExpr};\n`);
              accVars[outKey] = accVar;
            }
            this.mainCode.push(result.code);
            for (const [outKey, accVar] of Object.entries(accVars)) {
              const freshVar = result.outputVars[outKey];
              if (freshVar) this.mainCode.push(`    ${accVar} ${node.assignOp} ${freshVar};\n`);
            }
            this.nodeOutputs.set(node.id, accVars);
          } else {
            this.mainCode.push(result.code);
            this.nodeOutputs.set(node.id, result.outputVars);
          }
        }
