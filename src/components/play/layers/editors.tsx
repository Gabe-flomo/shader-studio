...
import { CameraChip, LiveAudioChip } from '../chips';
...
const FONTS: Choice[] = [{ value: 'sans', label: 'Sans' }, { value: 'serif', label: 'Serif' }, { value: 'mono', label: 'Mono' }];
...
export function NullEditor({ f, ctx }: { f: FieldKit; ctx: EditorContext }) {
  ...
  return (
    <>
      ...
      ], 'A following null chases its target on a spring: it lags, overshoots and settles. Its X and Y (and anything mapped from them) move with that motion.')}
      {l.follow === 'null' && f.pick('Target', 'followId', nulls(ctx, l.id), 'Add a second null', 'The null this one chases.')}
  ...
  return (
    <>
      ...
      {f.row('Font', <>
        ...
        <NumberInput value={f.get<number>('weight')} min={100} max={900} step={100} title="Weight (400 regular, 700 bold)" onCommit={n => f.set({ weight: Math.max(100, Math.min(900, Math.round(n / 100) * 100)) })} style={{ ...f.numStyle, width: 48 }} />
      </>, 'Typeface and weight.')}
      {f.props('x', 'y', 'size', 'rotation', 'opacity')}
      {matteRows(f, pictureHidden)}
      <Section title="Sequence" />
      {f.toggle('Lines', 'sequence', 'Show one line at a time', 'Each line of the text is a step. A Next action (on a key, a beat, a note…) moves on; Every sets a steady pace.')}
      {seq && (
        <>
...
export function ImageEditor({ f, pictureHidden }: { f: FieldKit; pictureHidden: boolean }) {
  return (
    <>
      ...
      {f.props('x', 'y', 'scale', 'rotation', 'opacity')}
      {matteRows(f, pictureHidden)}
    </>
  );
}
...
export function ParticlesEditor({ f, ctx }: { f: FieldKit; ctx: EditorContext }) {
  ...
  const usesNull = g('attractor') === 'null' || g('spawn') === 'null' || g('sizeBy') === 'null' || g('opacityBy') === 'null';
  ...
  return (
    <>
      ...
      <Section title="Flocking" />
      {f.prop('flock')}
      {g<number>('flock') > 0 && f.props('flockRadius', 'flockAlign', 'flockCohere', 'flockSeparate')}
      {g<number>('flock') > 0 && f.note('Boids: each particle also steers by its neighbours. It still follows the field, the mouse and the zones; turn Flock up for the flock to win.')}
      {f.prop('collide')}
      ...
      ], 'What happens when a particle leaves the picture: wrap round, bounce back, or respawn where particles are born.')}
      {f.props('life', 'fade')}
      ...
      <Buttons>
        ...
        <Button size="sm" variant="ghost" onClick={() => ctx.act('reset')}>Reset</Button>
      ...
      {usesNull && (
        <>
          {f.pick('Null', 'nullId', nulls(ctx), 'Add a Null layer first', 'The null the attractor, the spawn point and “nearness to a null” use.')}
          {(g('sizeBy') === 'null' || g('opacityBy') === 'null') && f.prop('falloff')}
        </>
      )}
...
export function ShapeEditor({ f, ctx }: { f: FieldKit; ctx: EditorContext }) {
  ...
  return (
    <>
      ...
      ], 'Box, circle and line are sized with the sliders. Drawn is any outline you click or drag on the picture. Layer takes a text or image layer\'s shape (particles flow around your words); Picture makes the bright parts of the shader solid.')}
      {shape === 'polygon' && (
        <Buttons>
          {ctx.drawing
      ...
      {geometric && f.props('x', 'y', ...(shape === 'polygon' ? [] : ['w', 'h']), 'rotation', ...(shape === 'box' ? ['round'] : []))}
      {geometric && f.note('Drag it on the picture while this tab is open.')}
      ...
      <Section title="Look" />
      {f.toggle('Show', 'show', 'Draw the shape', 'Off makes it an invisible zone: it still acts on particles, and is outlined only while this tab is open.')}
      {g<boolean>('show') && (
        <>
          {f.colour('Fill', 'fill')}
          {f.prop('fillOpacity')}
...
export function AudioEditor({ f }: { f: FieldKit }) {
  return (
    <>
      {f.row('Input', <LiveAudioChip />, 'The live audio input: a mic, an audio interface, or your DAW through a virtual cable (see the Connect guide).')}
      {f.seg('Style', 'style', [
      ...
      {f.get('style') === 'bars' || f.get('style') === 'ring'
        ? f.row('Bars', <NumberInput value={f.get<number>('bars')} min={4} max={256} step={4} title="How many bands" onCommit={n => f.set({ bars: Math.max(4, Math.min(256, Math.round(n))) })} style={f.numStyle} />)
        : null}
      {f.toggle('Mirror', 'mirror', f.get('style') === 'bars' ? 'Grow from the middle' : 'Draw it twice, flipped')}
      {f.props('x', 'y', 'w', 'h', 'gain', 'smooth', 'thickness')}
...
export function GlyphsEditor({ f }: { f: FieldKit }) {
  const style = f.get<string>('style'), colour = f.get<string>('colour');
  return (
    <>
      {f.seg('Style', 'style', [
      ...
      {style === 'ascii' && f.row('Characters', <Field value={f.get<string>('chars')} onChange={e => f.set({ chars: e.target.value })} height={26} mono style={{ flex: 1, minWidth: 0 }} />, 'From dark to bright. A space leaves the darkest cells empty.')}
      {f.props('cell', 'contrast')}
      {f.toggle('Invert', 'invert', 'Bright parts get the small glyphs')}
      {f.seg('Reads', 'readFrom', READ_FROM)}
      {f.seg('Colour', 'colour', [{ value: 'picture', label: 'Picture' }, { value: 'tint', label: 'Tint' }, { value: 'palette', label: 'Palette' }], 'Picture colours each glyph by the picture under it.')}
...
export function LensEditor({ f, ctx }: { f: FieldKit; ctx: EditorContext }) {
  return (
    <>
      ...
      {f.get('follow') === 'null' && f.pick('Null', 'nullId', nulls(ctx), 'Add a Null layer first')}
      {f.props(...(f.get('follow') === 'none' ? ['x', 'y'] : []), 'radius', 'amount', 'ring')}
...
export function BrushEditor({ f, ctx }: { f: FieldKit; ctx: EditorContext }) {
  return (
    <>
      ...
      {f.get('paint') === 'null' && f.pick('Null', 'nullId', nulls(ctx), 'Add a Null layer first')}
      {f.props('size', 'fade')}
...
export function BodiesEditor({ f, ctx }: { f: FieldKit; ctx: EditorContext }) {
  ...
  return (
    <>
      ...
      {source === 'letters' && f.seg('Font', 'font', FONTS)}
      {f.props('size', 'gravity', 'angle', 'bounce', 'friction')}
